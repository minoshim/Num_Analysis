...
int main(void)
{
  ...
  const double kk=k0*2.0;	// Perturbation wavenumber
  const double kj=kk/0.5;	// Jeans wavenumber
  const double vs=sqrt(4.0*M_PI)/kj; // Thermal velocity
  ...
  while(n++ < nmax){
	...
	g[i]=g[i-1]-4.0*M_PI*drho[i-1]*dx;
      }
      double gmean=0.0;
      for (i=xoff;i<nx-xoff;i++){
    ...
    for (i=xoff;i<nx-xoff;i++){
      ...
      csl3rd(ftmp,0.5*(g[i]+g[i+1]),dt,dv,nv,voff);
